...
```sh
# the future scheduler in one line: this container runs on exactly one node
...
conch elect pg-primary --on-acquire "pg_ctl promote" --on-lose "pg_ctl demote" --restart -- postgres

# debug: who currently holds the patroni-failover office?
conch elect patroni-failover --who

# check if this host is the active leader with a minimum term of 100
if conch elect patroni-failover --assert --min-rev 100; then
  echo "I am the active primary"
fi

# dashboard: watch all leadership churn for an office
conch elect incus-frobnitz --watch
```
