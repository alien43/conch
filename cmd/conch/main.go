...
Global Env Options (can be passed as flags too):
  CONCH_ENDPOINTS (default: asus-server:2379,dell-server:2379,rpi-server:2379)
...
func registerGlobalFlags(fs *flag.FlagSet) (endpointsStr *string, dialTimeoutStr *string, ttlStr *string, quietFlag *bool) {
	endpointsStr = fs.String("endpoints", getEnvOrDefault("CONCH_ENDPOINTS", "asus-server:2379,dell-server:2379,rpi-server:2379"), "comma-separated etcd endpoints")
